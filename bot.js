...
const maraya = require('maraya-public-2');
...
const maraya = config.DATABASE.define('maraya', {
    info: {
...
async function maraya () {
    await config.DATABASE.sync();
    var StrSes_Db = await marayaDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
...
const marayaCon = new WAConnection();
const Session = new StringSession();
marayaCon.version = [2, 2126, 14]
setInterval(async () => { 
        ...
        await axios.get('https://gist.githubusercontent.com/yasasdileepa/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (infoen !== '' && config.LANG == 'EN' || config.LANG == 'ES') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return marayaCon.sendMessage(marayaCon.user.jid, '[ ```🔔maraya Announcements🔔``` ]\n\n' + infoen.replace('{user}', QueenAmdiCon.user.name).replace('{wa_version}', marayaCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', maraya.user.phone.os_version).replace('{device_model}', marayaCon.user.phone.device_model).replace('{device_brand}', marayaCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (infosi !== '' && config.LANG == 'SI') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return marayaCon.sendMessage(marayaCon.user.jid, '[ ```🔔maraya නිවේදන🔔``` ]\n\n' + infosi.replace('{user}', QueenAmdiCon.user.name).replace('{wa_version}', maraya.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', marayaCon.user.phone.os_version).replace('{device_model}', marayaCon.user.phone.device_model).replace('{device_brand}', marayaCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
}, 50000);
    ...
    setInterval(async () => { 
        ...
        await axios.get('https://gist.githubusercontent.com/yasasdileepa/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (verify !== '' && config.LANG == 'EN') {
                ...
                while (getGMTh == 04 && getGMTm == 55) { 
                    return mrayaCon.sendMessage(QueenAmdiCon.user.jid, HANDLER + verify.replace('{user}', marayaCon.user.name).replace('{wa_version}', marayaCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', marayaCon.user.phone.os_version).replace('{device_model}', marayaCon.user.phone.device_model).replace('{device_brand}', marayaCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
    }, 50000);
...
// ======================maraya_Logger=======================
marayaCon.logger.level = config.DEBUG ? 'debug' : 'warn';
var nodb;

    if (StrSes_Db.length < 1) {
        nodb = true;
        marayaCon.loadAuthInfo(Session.deCrypt(config.SESSION)); 
    } else {
        MARAYACon.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }

if (config.LANG == 'EN' || config.LANG == 'ES') {
    MARAYACon.on ('credentials-updated', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login details updated!')
        );

        const authInfo = marayaCon.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
            await marayaDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
if (config.LANG == 'EN' || config.LANG == 'ES') {
    QueenAmdiCon.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('🛡️ maraya Sheild Activated!')
        );
        await new Promise(r => setTimeout(r, 1100));

if (config.WORKTYPE == 'public') {
    if (marayaCon.user.jid === '@s.whatsapp.net') {

        await marayaCon.sendMessage(QueenAmdiCon.user.jid, '```🛡️ Blacklist Detected!``` \n```User:```  \n```Reason:``` ', MessageType.text)
        ...
        await marayaCon.sendMessage(marayaCon.user.jid, '*maraya started in Public Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Public Mode. To change, use_ ```.setvar WORK_TYPE=private``` _command._\n\n*You can check our bot for new features : *\n\n*Comment here to request new features:*\n\n\n*Updates:* https://gist.github.com/yasasdileepa/890b6b31fcb8d376d6a68afcb7359324\n\n *Thank you for using King maraya 💌*', MessageType.text);
    }
}
else if (config.WORKTYPE == 'private') {
    if (marayaon.user.jid === '@s.whatsapp.net') {

        await marayaCon.sendMessage(marayaCon.user.jid, '```🛡️ Blacklist Detected!``` \n```User:```  \n```Reason:``` ', MessageType.text)
        ...
        await marayaCon.sendMessage(maraya.user.jid, '*maraya started in Private Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Private Mode. To change, use_ ```.setvar WORK_TYPE=public``` _command._\n\n*You can check our bot for new features : /*\n\n*Comment here to request new features:*\n\n\n*Updates:* https://gist.github.com/yasasdileepa/890b6b31fcb8d376d6a68afcb7359324\n\n *Thank you for using maraya 💌*', MessageType.text);
    }
}
    else {
    ...
    marayaCon.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
                ...
                if (marayaCon.user.jid === '@s.whatsapp.net') {
                    ...
                    await marayaCon.sendMessage(marayaCon.user.jid, '```🛡️ Blacklist අනාවරණය විය!``` \n```පරිශීලක:``` \n```හේතුව:``` ', MessageType.text)
                ...
                if (marayaCon.user.jid === '@s.whatsapp.net') {
                    ...
                    await QueenAmdiCon.sendMessage(marayaCon.user.jid, '```🛡️ Blacklist Detected!``` \n ```පරිශීලක:``` \n```හේතුව:``` ', MessageType.text)
...
// ==============================================================
    ...
    marayaCon.on('message-new', async msg => {
        if (msg.key && msg.key.remoteJid == 'status@broadcast') return;

        if (config.NO_ONLINE) {
            await marayaCon.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        ...
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                await marayaCon.sendMessage(msg.key.remoteJid, Buffer.from (blogo.data), MessageType.image, {mimetype: Mimetype.png, caption: gb.message});
            }
            return;
            ...
            if (gb !== false) {
                await marayaCon.sendMessage(msg.key.remoteJid, Buffer.from (wlogo.data), MessageType.image, {mimetype: Mimetype.png, caption: gb.message});
            }
            return;
        }
        // ====================================================================
        ...
        if (config.SUPPORT1 == '9475881391-1631633729') {     
            var tsup = config.SUPPORT1.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (config.SUPPORT2 == '947588131191-1631905677') {     
        ...
        if (config.RRBSEW == '9475881391-1625490851') {     
        ...
        if (config.RRDSEW == '9470379493-1626432386') {     
        ...
        if (config.SEWSUPPORT == '94710995750-1627812354') {     
        ...
        if (config.SEWSUPPORT2 == '94758813191-1628835469') {     
        ...
        if (config.SEWSUPPORT3 == '9475883191-1628835633') {     
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = marayaCon.chats.get(msg.key.remoteJid)
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await marayaCon.chatRead(msg.key.remoteJid);
                        }
                        ...
                        && msg.message.imageMessage !== null) {
                            whats = new Image(marayaCon, msg);
                        } else if (command.on !== undefined && (command.on === 'video' )
                        && msg.message.videoMessage !== null) {
                            whats = new Video(marayaCon, msg);
                        } else {
                            whats = new Message(marayaCon, msg);
                        }
                        if (msg.key.fromMe) {
                            var vers = marayaCon.user.phone.wa_version.split('.')[2]
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await marayaCon.sendMessage(MarayaCon.user.jid, '*-- දෝෂ වාර්තාව [KING MARAYA] --*' + 
                                    '\n\n*MARAYA Bot දෝෂයක් සිදුවී ඇත!*'+
                                    ...
                                    '\n_PLEASE LOOK OWER GITHUB WEB SITE' +
                                    '\n\n_මෙම පණිවිඩය ඔබගේ අංකයට ගොස් තිබිය යුතුය (සුරකින ලද පණිවිඩ)._\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await marayaCon.sendMessage(MARAYACon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [MARAYA] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                    ...
                                    return await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [MARAYA] ⚜️*' + 
                                        '\n========== ```දෝෂ නිරාකරණය කර ඇත``` ==========' +
                                    ...
                                    return await marayaCon.sendMessage(marayaCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [MARAYA] ⚜️*' + 
                                        ...
                                        '\n\n*විසඳුම:* _මිනිත්තු කිහිපයක් රැඳී සිටින්න. yasasdileepa විසින් මෙම දෝෂය නිවැරදි කරනු ඇත._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await QueenAmdiCon.sendMessage(marayaCon.user.jid, '*🙇🏻 කණගාටුයි, මට මෙම දෝෂය කියවිය නොහැක! 🙇🏻*' +
                                        '\n_වැඩිදුර උදව් සඳහා ඔබට අපගේ support කණ්ඩායම් වෙත ලිවිය හැකිය._'
                                        , MessageType.text
                                    );
                                }
                            } else {
                                await QueenAmdiCon.sendMessage(marayaCon.user.jid, '*-- ERROR REPORT [MARAYA] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await marayaCon.sendMessage(marayaCon.user.jid, '*⚜️ ERROR ANALYSIS [MARAYA] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await marayaCon.sendMessage(marayaCon.user.jid, '*⚜️ ERROR ANALYSIS [MARAYA] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                        '\n\n*Main Error:* _Ookla Server marayaConection_' +
                                    ...
                                    return await marayaCon.sendMessage(QmarayaCon.user.jid, '*⚜️ ERROR ANALYSIS [MARAYA] ⚜️*' + 
                                        '\n========== ```Error Resolved``` ==========' +
                                    ...
                                    return await marayaCon.sendMessage(marayaCon.user.jid, '*⚜️ ERROR ANALYSIS [MARAYA] ⚜️*' + 
                                        ...
                                        '\n\n*Reason:* _marayaConection between site dismarayaConected._' +
                                    ...
                                    return await marayaCon.sendMessage(QueenAmdiCon.user.jid, '*⚜️ ERROR ANALYSIS [maraya] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await MARAYACon.sendMessage(marayaCon.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
